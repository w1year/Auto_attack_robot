...
std::atomic<int> g_currentPicAngle(15000);
    ...
    while (g_running.load()) {
        try {
            ...
            if (direction > 0 && distanceToLeft < 500) {
                 currentSpeed = std::max(10, static_cast<int>(patrolSpeed * (distanceToLeft / 500.0)));
            } else if (direction < 0 && distanceToRight < 500) {
                 currentSpeed = std::max(10, static_cast<int>(patrolSpeed * (distanceToRight / 500.0)));
            }
...
void detectionThread(MVSCamera* camera, YOLODetectorTensorRT* detector, GimbalController* gimbal) {
    ...
    // 跟踪参数
    int stepSize = 40;       // 基本步长
    int centerMargin = 75;  // 中心区域范围
    ...
    while (g_running.load()) {
        try {
            ...
            if (!targetDetections.empty()) {
                ...
                int yDeviation = targetCenterY - (frame.rows / 2);
                // 判断是否在中心区域
                bool centerStableX = std::abs(xDeviation) <= centerMargin;
				bool centerStableY = std::abs(yDeviation) <= 200; 
				bool centerStable = centerStableX && centerStableY;
                ...
                // 云台控制 (基于屏幕 X/Y 偏移量)
                {
                    // 将 X 轴和 Y 轴的更新放在同一个互斥锁内，防止多线程冲突
                    std::lock_guard<std::mutex> lock(g_gimbalMutex);

                    // --- 俯仰角 (Pitch) 控制 ---
                    if (!centerStableY) {
                        // 修正1：不再读取物理滞后角度，读取全局软件期望值
                        // 修正2：将 5000 的离谱步长改为和 Yaw 一样的 stepSize (40)
                        int newPicAngle = g_currentPicAngle.load() + 
                                          (yDeviation > 0 ? -stepSize : stepSize);
                        
                        // 严格限幅在 0 - 30000 之间
                        newPicAngle = std::max(0, std::min(30000, newPicAngle));
                        gimbal->setPicAngle(newPicAngle);
                        
                        // 修正3：更新全局期望值，实现正确累加追踪
                        g_currentPicAngle = newPicAngle; 
                    }

                    // --- 偏航角 (Yaw) 控制 ---
                    if (!centerStableX) {
                        // 偏航角依然采用你原有的定步长追踪逻辑，也可改为比例步长
                        int newYawAngle = g_currentYawAngle.load() + 
                                          (xDeviation > 0 ? -stepSize : stepSize);
                        
                        // 严格限幅在 0 - 30000 之间
                        newYawAngle = std::max(0, std::min(30000, newYawAngle));
                        gimbal->setYawAngle(newYawAngle);
                        g_currentYawAngle = newYawAngle;
                    }
                }
...
//    selectTargetType();
try {
        // 1. [提前] 初始化云台 (因为需要通过CAN总线读取阵营信息)
        LOG_INFO("正在初始化云台控制与CAN总线...");
        ...
        // 2. 阻塞等待阵营信息 (CAN ID: 0x610)
        LOG_INFO("等待底层下发阵营信息(CAN ID: 0x610)...");
        while (g_running.load()) {
            uint8_t code = gimbal.getTargetColorCode();
            if (code == 1) {
                g_targetType = "red";
                LOG_INFO("已确认阵营: 红方目标 (Red)");
                break;
            } else if (code == 2) {
                g_targetType = "blue";
                LOG_INFO("已确认阵营: 蓝方目标 (Blue)");
                break;
            }
            // 如果是 0，说明还没收到或者收到的就是0，继续休眠等待
        ...
        // 4. 加载模型 - 根据刚刚动态读取到的 g_targetType 决定
